...
- Install (local): `Rcmd.exe INSTALL --preclean --no-multiarch --with-keep.source .`

## Known Pitfalls and Fixes
- renv:
  - `.Rprofile` must only source `renv/activate.R` in interactive sessions.
- Quarto:
...
- Non-source artifacts (e.g., `my_animated_bar.gif`) are in `.Rbuildignore`.
...
- Declare globals for NSE (`rlang::.data` or globals).
